---
name: codex
description: Delegate tasks to OpenAI Codex (GPT-5.6 Sol) as background tasks for precision coding, code review, deliberation, and complex implementation. Always launch in background (run_in_background=true), continue working, then collect results with TaskOutput when needed.
...
Canonical: ~/.claude/skills/codex (private, live). Public sanitized twin: ~/code/skills/codex → github.com/justinwilliames/skills. Sanitization is a sync step — never push private paths/names.
